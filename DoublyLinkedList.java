...
public class DoublyLinkedList {
	...
	public void insert(int ele) {
		if(start==null) {		//first time 
			start=new Node(null,ele,null);
		}
		else {
			ptr=start;			//other
			while(ptr.getNext()!=null)
				ptr=ptr.getNext();
			ptr.setNext(new Node(ptr,ele,null));
		}
	}
	public void insert_begin(int ele)	{
		if(start==null) {
			...
			start.setPre(new Node(null,ele,start));
			start=start.getPre();
		}
	}
	...
	public void insert_end(int ele)	{
		if(start==null) {
			start=new Node(null,ele,null);//if list is empty
		}
		else {
			ptr=start;//moving to last node
			while(ptr.getNext()!=null)
				ptr=ptr.getNext();
			ptr.setNext(new Node(ptr,ele,null));
		}
	...
	public void insert_par(int par,int ele) throws Exception	{
		...
		if(start.getVal()==par)	{//if par is at first
			insert_begin(ele);
			return;
		}
		preptr=ptr=start;//if element is to be inserted other than first
		...
		preptr.setNext(new Node(preptr,ele,ptr));
		ptr.setPre(preptr.getNext());
	}
	...
	public void delete_end() throws Exception	{
		if(start==null) {
			...
			preptr=ptr=start;
			while(ptr.getNext()!=null){//moving last node
				preptr=ptr;
				ptr=ptr.getNext();
			}
			preptr.setNext(null);
	...
	public int count() {
		if(start==null)
			...
			ptr=start;
			int count=0;
			while(ptr!=null) {//same as display()
				count++;
				ptr=ptr.getNext();
			}
			return count;
	...
	public void delete_par(int par) throws Exception {
		if(start==null) {
			...
			if(start.getVal()==par)	{//if ele is first
				delete_begin();
				return;
			}
			preptr=ptr=start;
			while(ptr.getVal()!=par && ptr.getNext()!=null) {//moving till element
				preptr=ptr;						//else traverse till last
				ptr=ptr.getNext();
			}
			if(ptr.getVal()!=par) {
				throw new Exception("element not found!!!");
